...
// GetPostVoteData 根据Redis查询每篇帖子投赞成票的数据
...
func GetCommunityPostIDsInOrder(p *models.ParamCommunityPostList) ([]string, error) {
	...
	if p.Order == models.OrderScore {
		orderKey = getRedisKey(KeyScorePostZSet)
	}
	...
	//lion给缓存key减少zinterstore执行的次数
	key := orderKey + strconv.Itoa(int(p.CommunityID))
	if rdb.Exists(key).Val() < 1 {
		//如果第一次存进来的时候不存在 需要计算
		pipeline := rdb.Pipeline()
		pipeline.ZInterStore(key, redis.ZStore{
			Aggregate: "MAX", //两个ZSet聚合的时候的函数是什么
		}, cKey, orderKey) //zinterstore计算
		pipeline.Expire(key, 60*time.Second) //设置超时时间(60秒之内再来访问这个帖子)
		_, err := pipeline.Exec()
		if err != nil {
			return nil, err
		}
